...
object RegExDemo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var stringToFind = "-1.5 divide by 5 is 3 is wrong"

    for (decimal(sign, integer, deci) <- decimal findAllIn stringToFind ) {
      println("Sign= "+ sign + "Integer= "+ integer + "Decimal= "+ deci )
    }
